...
pub fn parse_droid_file(path: &Path) -> Vec<UnifiedMessage> {
    ...
    let totals = TokenBreakdown {
        ...
        // Droid reports no 1-hour cache-write split.
        cache_write_1h: 0,
    };
...
mod tests {
    ...
    #[test]
    fn test_conservation_across_all_token_lanes() {
        // Brief requirement: the per-reply records of one session sum to the
        // settings file's tokenUsage on every token lane, not just input.
        let temp_dir = tempfile::TempDir::new().unwrap();
        let settings = write_session(
            temp_dir.path(),
            "cccccccc-0000-0000-0000-000000000000",
            r#"{"inputTokens":123456,"outputTokens":7890,"cacheCreationTokens":4321,"cacheReadTokens":65432,"thinkingTokens":999}"#,
            &[
                &user("2026-08-07T10:00:00Z", 1000),
                &assistant("2026-08-07T11:00:00Z", 500),
                &assistant("2026-08-08T11:00:00Z", 30_000),
                &assistant("2026-08-09T11:00:00Z", 5),
            ],
        );

        let messages = parse_droid_file(&settings);

        assert_eq!(messages.iter().map(|m| m.tokens.input).sum::<i64>(), 123456);
        assert_eq!(messages.iter().map(|m| m.tokens.output).sum::<i64>(), 7890);
        assert_eq!(
        ...
        assert!(messages.iter().all(|m| m.tokens.cache_write_1h == 0));
    }
}
